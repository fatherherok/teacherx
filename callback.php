...
<?php include ('./inc/header.inc.php');
  ...
  mysqli_query($con,"UPDATE ordered_course SET paid = 'yes', reference='$reference' WHERE invoice = '$invoice' ") ;
 $success = 'You have successfully completed your payment. Please go to your dashboard to get the materials'; 
 


$gett = mysqli_query($con,"SELECT * FROM ordered_course WHERE invoice = '$invoice' ") or die (mysqli_error()); 
            $row=mysqli_fetch_array($gett);          
                $course_id = $row['course_id'];
                $date = $row['time_date'];

 $get1 = mysqli_query($con,"SELECT * FROM course WHERE id = '$course_id' ") or die (mysqli_error()); 
