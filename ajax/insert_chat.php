...
$url = '~(?:(https?)://([^\s<]+)|(www\.[^\s<]+?\.[^\s<]+))(?<![\.,:])~i'; 
  $message = mysqli_real_escape_string($con,htmlentities(trim(strip_tags($message))));
  $message = preg_replace($url, '<a href="$0" target="_blank" title="$0">$0</a>', $message);


$query= mysqli_query($con,"INSERT INTO chat_message VALUES ('', '$to', '$from', '$message', '$time', '$status', '', '$image_name', 'on')");	
