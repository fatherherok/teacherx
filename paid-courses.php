...
<?php include ('./inc/header.inc.php');  ?>   
      <h2 class="mb-3 bread text-center"><?php echo ucfirst($username); ?> Paid Courses</h2>
...
<script type="text/javascript">
  ...
  $(document).ready(function(){
	...
	<?php
                              ...
                              $get_co = mysqli_query($con,"SELECT ordered_id FROM ordered_course  ") or die (mysqli_error());
...
$get = mysqli_query($con,"SELECT *
  FROM `course`
  LEFT JOIN `ordered_course`
  ON `course`.`id` = `ordered_course`.`course_id`
  WHERE `ordered_course`.`username` = '$username' "); 
...
$gett = mysqli_query($con,"SELECT *
  ...
  LEFT JOIN `ordered_course`
  ON `course`.`id` = `ordered_course`.`course_id`
  WHERE `ordered_course`.`username` = '$username'   ORDER BY `ordered_course`.`ordered_id` DESC LIMIT $row_start, $limit  "); 
...
// $get_query = mysqli_query($con,"SELECT * FROM blog ORDER BY blog_id DESC");  ?>
                ...
                ?>
							...
							<div class="col-lg-6 course_col">
								<div class="course">
									<div class="course_image"><a href="paid-course.php?id=<?php echo $id; ?>"><img src="<?php echo $post_pix; ?>" alt="" width="100%"></a></div>
									<div class="course_body">
										<h3 class="course_title"><a href="paid-course.php?id=<?php echo $id; ?>"><?php echo $title; ?></a></h3>
										<div class="course_teacher">Course Code: <b><?php echo $pin; ?></b></div>
										<div class="course_teacher">Category: <b><?php echo $category; ?></b></div>
										<div class="course_text">
											<p><?php echo substr($description,0,100); ?>... <a href="paid-course.php?id=<?php echo $id; ?>" class="" style="">Check Out</a></p>
